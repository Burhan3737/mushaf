...
export default function StyleControls({ config, dispatch }: StyleControlsProps) {
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm text-amber-400 font-mono">{config.fontSize}px</span>
        </div>
        <input
          type="range"
          min={24}
          ...
          onChange={(e) =>
            dispatch({ type: 'SET_FONT_SIZE', payload: parseInt(e.target.value) })
          }
          ...
          {PRESET_COLORS.map((c) => (
            <button
              ...
              className={`w-8 h-8 rounded-full border-2 transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-amber-500
                ${config.color === c.value ? 'border-amber-500 scale-110' : 'border-neutral-600'}`}
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex gap-2">
          {ALIGN_OPTIONS.map((opt) => (
            <button
              key={opt.value}
              onClick={() => dispatch({ type: 'SET_ALIGN', payload: opt.value })}
              title={opt.label}
      ...
      <div className="space-y-1.5">
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm text-amber-400 font-mono">{config.lineHeight.toFixed(1)}×</span>
        </div>
        <input
          type="range"
          min={1.2}
          ...
          onChange={(e) =>
            dispatch({ type: 'SET_LINE_HEIGHT', payload: parseFloat(e.target.value) })
          }
          className="w-full accent-amber-500 cursor-pointer"
        />
        <div className="flex justify-between text-xs text-neutral-600">
      ...
      <div className="space-y-1.5">
        ...
        <div className="flex gap-2">
          {CANVAS_WIDTHS.map((w) => (
            <button
              key={w}
              onClick={() => dispatch({ type: 'SET_CANVAS_WIDTH', payload: w })}
              className={`flex-1 py-2 rounded-lg border text-sm font-medium transition-all focus:outline-none focus:ring-2 focus:ring-amber-500
        ...
        <div className="text-xs text-neutral-600 text-center">
          {config.canvasWidth === 1080 && 'Square / Portrait'}
          {config.canvasWidth === 1920 && 'Full HD (16:9)'}
          {config.canvasWidth === 2560 && '2K / Wide'}
        </div>
      </div>
    </div>
  );
