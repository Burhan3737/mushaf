...
export default function HomePage() {
  ...
  const { loadFont, isFontLoaded, isFontLoading, preloadFonts } = useFontLoader();
  ...
  useEffect(() => {
    ...
    preloadFonts(['pdms-saleem', 'amiri-quran', 'kfgqpc-hafs', 'amiri', 'scheherazade', 'noto-naskh', 'al-mushaf', 'me-quran']);
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-amber-500/20 flex items-center justify-center">
              <span className="text-amber-400 text-sm font-bold" style={{ fontFamily: 'serif' }}>م</span>
            </div>
            <div>
              <h1 className="text-sm font-semibold text-white">Mushaf Text</h1>
              <p className="text-xs text-neutral-500">Arabic overlay generator</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-screen-xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6 items-start">
          ...
          <div className="lg:sticky lg:top-20">
            <CanvasPreview config={config} isFontLoaded={isFontLoaded} isFontLoading={isFontLoading} />
            <div className="mt-4">
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-4 space-y-5">
              ...
              <FontPicker
                ...
                onSelect={(fontId) => dispatch({ type: 'SET_FONT', payload: fontId })}
                loadFont={loadFont}
